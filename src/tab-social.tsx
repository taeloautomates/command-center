...
function KPITile({ platform, value, label, delta, positive, icon }: {
  platform: string; value: string; label: string; delta: string; positive: boolean; icon: React.ReactNode;
}) {
  return (
    <GlassCard style={{ padding: 18, flex: 1 }} clickable>
      ...
      <Row gap={6} align="center">
        ...
        <span style={{ fontSize: 11, color: "rgba(255,255,255,0.32)" }}>· 7d</span>
      </Row>
    </GlassCard>
  );
}
...
export function TabSocial({ manual }: { manual: ManualState }) {
  const s = manual.social;
  return (
    <div className="surface" style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", gap: 12 }}>
      <Row gap={12} align="stretch">
        <KPITile platform="YouTube"   label="subs"      value={s.youtube.value}   delta={s.youtube.delta}   positive={s.youtube.positive}   icon={<YouTubeIcon size={12} opacity={0.6} />} />
